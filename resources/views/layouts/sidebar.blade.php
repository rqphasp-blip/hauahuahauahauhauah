...
<html>
  <head>
    ...
    <style>
        /* Main container for the mobile secondary menu */
        #menumob.nav-scroller {
            /* display: block; /* This will be controlled by removing/adding .d-none */
            position: relative; /* Stays in normal document flow */
            z-index: 1010; /* Ensure it's reasonably high, but below modals etc. Navbar is often 1030. */
            width: 100%;
            background-color: var(--bs-body-bg, #fff); /* Use CSS variable for background */
            border-top: 1px solid var(--bs-border-color-translucent, rgba(0,0,0,0.15));
            border-bottom: 1px solid var(--bs-border-color-translucent, rgba(0,0,0,0.15));
            height: auto; /* Critical: Override Bootstrap's .nav-scroller fixed height */
            overflow-y: auto; /* Critical: Allow vertical scrolling for content */
            max-height: calc(60vh); /* Adjust max height as needed, e.g., 60% of viewport height */
            padding: 0;
            margin: 0;
            clear: both; /* Ensure it clears any floated elements from navbar if any were used (unlikely with flex) */
        }

        /* Hide it by default using Bootstrap class, JS will toggle this */
        #menumob.d-none {
            display: none !important;
        }
        
        /* Styling for the <nav> element inside #menumob */
        #menumob .nav {
            display: flex;
        ...
        #menumob .nav-link {
            display: flex; 
            align-items: center; 
            padding: 0.85rem 1.25rem; /* Slightly more padding for touch targets */
            text-decoration: none;
            border-bottom: 1px solid var(--bs-border-color-translucent, rgba(0,0,0,0.1)); 
            color: var(--bs-body-color); /* Use CSS variable for text color */
            transition: background-color 0.15s ease-in-out, color 0.15s ease-in-out; /* Smooth transition for hover */
        }
        #menumob .nav-link:last-child {
            border-bottom: none; /* No border for the last item */
        }

        #menumob .nav-underline .nav-link:hover,
        #menumob .nav-underline .nav-link:focus {
             background-color: var(--bs-tertiary-bg); /* Use CSS variable for hover background */
             color: var(--bs-emphasis-color); /* Use CSS variable for hover text color */
        }
        
        #menumob .nav-underline .nav-link.active {
            font-weight: 600; /* Make active link bolder */
            color: var(--bs-primary); /* Use theme's primary color for active link */
            background-color: var(--bs-secondary-bg); /* Subtle background for active state */
            border-left: 3px solid var(--bs-primary); /* Active indicator */
            padding-left: calc(1.25rem - 3px); /* Adjust padding for border */
        }

        #menumob .nav-link i.bi { 
            margin-right: 0.85rem; 
            font-size: 1.15rem; 
            width: 22px; /* Ensure consistent icon width for alignment */
            text-align: center;
        }

        .header-block{background-color:var(--bs-primary);border-bottom-left-radius:1rem;border-bottom-right-radius:1rem;}

        /* Ensure custom toggler bars are visible if Bootstrap's default SVG is hidden */
        .iq-navbar .navbar-toggler-icon {
                   ...
                   <li class="nav-item">
                    ...
                    <li class="nav-item">
                        <a class="nav-link {{ (Request::segment(1) == 'admin' && Request::segment(2) == 'stories') || Request::is('stories*') ? 'active' : ''}}" href="{{ route('stories.index') }}" >
                            <i class="icon">
                                <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.5 4.5C3.5 3.94772 3.94772 3.5 4.5 3.5H19.5C20.0523 3.5 20.5 3.94772 20.5 4.5V19.5C20.5 20.0523 20.0523 20.5 19.5 20.5H4.5C3.94772 20.5 3.5 20.0523 3.5 19.5V4.5Z" stroke="currentColor" stroke-width="1.5"/><path d="M7.5 7.5H16.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/><path d="M7.5 11.5H16.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/><path d="M7.5 15.5H12.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>
                    ...
                    <li class="nav-item">
                        <a class="nav-link {{ Request::segment(1) == 'banner' ? 'active' : ''}}" href="{{ route('banner.index') }}">
                            <i class="icon">
                                 <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M21 19V5C21 3.89543 20.1046 3 19 3H5C3.89543 3 3 3.89543 3 5V19C3 20.1046 3.89543 21 5 21H19C20.1046 21 21 20.1046 21 19ZM8.5 10.5C7.67157 10.5 7 11.1716 7 12C7 12.8284 7.67157 13.5 8.5 13.5C9.32843 13.5 10 12.8284 10 12C10 11.1716 9.32843 10.5 8.5 10.5ZM15.5 14.5L13 11.5L10.5 14.5H18.5L15.5 14.5Z" fill="currentColor"/></svg>
                            </i>
                    ...
                    <li class="nav-item">
                        <a class="nav-link {{ Request::segment(1) == 'googlereviews' ? 'active' : ''}}" href="{{ route('googlereviews.index') }}">
                            <i class="icon">
                                <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 17.27L18.18 21L16.54 13.97L22 9.24L14.81 8.63L12 2L9.19 8.63L2 9.24L7.46 13.97L5.82 21L12 17.27Z" fill="currentColor"/></svg>
                            </i>
                            <span class="item-name">Avaliações Google</span>
                        </a>
                    </li>
                    <li class="nav-item">
      ...
      <div class="position-relative iq-banner">
        ...
        <nav class="nav navbar navbar-expand-lg navbar-light iq-navbar">
          <div class="container-fluid navbar-inner">
            ...
            <div class="sidebar-toggle" data-toggle="sidebar" data-active="true" id="sideicon1">
                <i class="icon">
              ...
              <ul class="mb-2 navbar-nav ms-auto align-items-center navbar-list mb-lg-0">
                <li class="me-0 me-xl-2">
                  <div class="dropdown d-flex flex-row align-items-center">
                    ...
                    <a target="_blank" href="{{'/@'.Auth::user()->littlelink_name}}">
		  ...
		  <div class="nav-scroller bg-body shadow-sm d-none" id="menumob">
        <nav class="nav nav-underline" aria-label="Secondary navigation">
            <a class="nav-link {{ Request::segment(1) == 'dashboard' ? 'active' : ''}}" href="{{ route('panelIndex') }}">
                <i class="bi bi-speedometer2"></i> Dashboard
            </a>
            <a class="nav-link" target="_blank" href="{{ url('/@'.$usrhandl) }}">
                <i class="bi bi-eye-fill"></i> {{__('messages.View Page')}}
            </a>
             <a class="nav-link {{ Request::segment(2) == 'add-link' ? 'active' : ''}}" href="{{ url('/studio/add-link') }}">
                <i class="bi bi-plus-circle-fill"></i> {{__('messages.Add Link')}}
            </a>
            <a class="nav-link {{ Request::segment(2) == 'links' ? 'active' : ''}}" href="{{ url('/studio/links') }}">
                <i class="bi bi-link-45deg"></i> {{__('messages.Links')}}
            </a>
            <a class="nav-link {{ (Request::segment(1) == 'admin' && Request::segment(2) == 'stories') || Request::is('stories*') ? 'active' : ''}}" href="{{ route('stories.index') }}" >
            ...
            <a class="nav-link {{ Request::segment(1) == 'banner' ? 'active' : ''}}" href="{{ route('banner.index') }}">
                <i class="bi bi-card-image"></i> Gerenciar Banner
            </a>
            <a class="nav-link {{ Request::segment(1) == 'googlereviews' ? 'active' : ''}}" href="{{ route('googlereviews.index') }}">
            ...
            <a class="nav-link {{ Request::segment(2) == 'theme' ? 'active' : ''}}" href="{{ url('/studio/theme') }}">
                <i class="bi bi-brush-fill"></i> {{__('messages.Themes')}}
            </a>
          ...
          <div class="footer-body">
              ...
              <div class="right-panel">
                ...
                @if(env('DISPLAY_CREDIT_FOOTER') === true)
                  <span class="">
                    - Service by
                      <a href="https://lgw.one" target="_blank">LGW.one</a>.
                @endif
              </div>
          </div>
      </footer>
...
$(document).ready(function() {
    const LG_BREAKPOINT = 992; // Bootstrap's lg breakpoint (for navbar-expand-lg)
    const $menuMob = $('#menumob');
    const $navbarToggler = $('.navbar-toggler'); 
    const $navbarSupportedContent = $('#navbarSupportedContent'); 
    const $as1 = $('#as1'); 
    const $sideicon1 = $('#sideicon1'); 

    function setLayoutBasedOnViewport() {
        var viewportWidth = $(window).width();
        if (viewportWidth < LG_BREAKPOINT) { // --- Mobile View ---
            $as1.hide();
            $sideicon1.hide();
            // Ensure user menu target of toggler is hidden on mobile
            // and its Bootstrap collapse instance is not 'shown'
            if ($navbarSupportedContent.hasClass('show')) {
                var bsCollapse = bootstrap.Collapse.getInstance($navbarSupportedContent[0]);
                if (bsCollapse) {
                    bsCollapse.hide(); // Use Bootstrap API to hide it, preventing animation issues
                } else {
                    $navbarSupportedContent.removeClass('show'); // Fallback if instance not found
                }
            }


            // Initialize #menumob and toggler state for mobile view
            if ($menuMob.hasClass('d-none')) {
                $navbarToggler.addClass('collapsed').attr('aria-expanded', 'false');
            } else {
                $navbarToggler.removeClass('collapsed').attr('aria-expanded', 'true');
            }

        } else { // --- Desktop View ---
            $as1.show();
            $sideicon1.show();
            $menuMob.addClass('d-none'); 
            
            // On desktop, #navbarSupportedContent should be visible as part of the navbar
    ...
    setLayoutBasedOnViewport();

    // Adjust layout on window resize
    $(window).on('resize', setLayoutBasedOnViewport);

    // Handle clicks on the navbar toggler
    $navbarToggler.on('click', function(event) {
        ...
        if (viewportWidth < LG_BREAKPOINT) { 
            ...
            // Ensure #navbarSupportedContent is definitely hidden and not trying to show
            if ($navbarSupportedContent.hasClass('show')) {
                 var bsCollapse = bootstrap.Collapse.getInstance($navbarSupportedContent[0]);
                if (bsCollapse) {
                    bsCollapse.hide();
      ...
      <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            if (downloadButton && generatedImage) { // Ensure elements exist
                downloadButton.addEventListener("click", function() {
                    var format = generatedImage.getAttribute("data-format") || "png"; // Should ideally get format from actual image type
                    var downloadLink = document.createElement("a");
                    downloadLink.href = generatedImage.src;
                    downloadLink.download = "qr_code." + format; // More descriptive filename
